...
export const useGameStats = (level: number, times: number[] , points: number[], variant : string ): [GameStats, AddLinesCleared, TimeChange] => {
  ...
  const addLinesCleared = useCallback((lines: number) => {
    setGameStats((previous) => {
      ...
      const jsonString = JSON.stringify({pointsRecord: score, timeRecord: previous.seconds});
  ...
  const timeChange = useCallback(() => {
    setGameStats((previous) => {
      ...
      const jsonString = JSON.stringify({pointsRecord: previous.score, timeRecord: seconds});
      localStorage.setItem('score', jsonString);
      return {
        variant: previous.variant,
        points: previous.points,
